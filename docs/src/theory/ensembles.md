...
`ClockEnsemble` models a precision oscillator as a linear stochastic
...
In ClockEnsemble the two-state SDE corresponds to [`TwoStateClock`](@ref).
...
In ClockEnsemble the three-state SDE corresponds to
[`ThreeStateClock`](@ref).
...
In ClockEnsemble, `Φ` is built by the
...
In ClockEnsemble, `Q` is built by the [`process_noise`](@ref) hook on
...
- [API: `ClockEnsemble`](../reference.md) — exported types and
  functions.
